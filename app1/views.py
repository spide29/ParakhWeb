...
def word_recording(request):
    if request.method == "POST":
        ...
        question = request.session.setdefault('question', [])
...
def word_answer(request):
    ...
    context = { 'transcript': transcript,'text': text, 'question':question ,'copy_file': copy_file, 'audio_urls': audio_urls ,'mis':mis ,'flu':wcpm,'total_wcpm': total_wcpm,'total_mis':total_mis}
    if len(audio_file)==5:
        audio_file.clear()
    if len(audio_file_name)==5:
        audio_file_name.clear()
...
def next_word(request):
    ...
    if request.method == "POST":
        ...
        if "record" in request.POST:
            ...
            request.session['dataid[1]'] = dataid[1]
            ...
            print("dataid0",dataid[2])
            request.session['dataid[2]'] = dataid[2]
            ...
            request.session['dataid[3]'] = dataid[3]
            ...
            request.session['dataid[4]'] = dataid[4]
                ...
                try:
                    ...
                    for i, path in enumerate(copy_file):
                        if path == file_path:
                            copy_file[i] = file_path
                    print(copy_file)
                    return render(request, "retake_word/retake_word_one.html", {"recording": True,"val":val})
            ...
            if response.status_code == 200:
                ...
                if len(responses)==5:
                    ...
                    if len(responses)==5:
                        ...
                        return redirect(url,context)
    return render(request, "retake_word.html", {"recording": True })
    ...
    if request.method == "POST":
        ...
        if "record2" in request.POST:
            ...
            with sr.Microphone() as source:
                ...
                audio = r.listen(source)
                try:
                    request.session['dataid[0]'] = dataid[1]
                    ...
                    for i, path in enumerate(copy_file):
                        if path == file_path:
                            copy_file[i] = file_path
                    print(copy_file)
                    return render(request, "retake_word/retake_word_one2.html", {"recording": True,"val":val})
    ...
    if request.method == "POST":
        ...
        if "record3" in request.POST:
            ...
            with sr.Microphone() as source:
                ...
                audio = r.listen(source)
                try:
                    request.session['dataid[0]'] = dataid[2]
                    ...
                    for i, path in enumerate(copy_file):
                        if path == file_path:
                            copy_file[i] = file_path
                    print(copy_file)
                    return render(request, "retake_word/retake_word_one3.html", {"recording": True,"val":val})
    ...
    if request.method == "POST":
        ...
        if "record4" in request.POST:
            ...
            with sr.Microphone() as source:
                ...
                audio = r.listen(source)
                try:
                    request.session['dataid[0]'] = dataid[3]
                    ...
                    for i, path in enumerate(copy_file):
                        if path == file_path:
                            copy_file[i] = file_path
                    print(copy_file)
                    return render(request, "retake_word/retake_word_one4.html", {"recording": True,"val":val})
    ...
    if request.method == "POST":
        ...
        if "record5" in request.POST:
            ...
            with sr.Microphone() as source:
                ...
                audio = r.listen(source)
                try:
                    request.session['dataid[0]'] = dataid[4]
                    ...
                    for i, path in enumerate(copy_file):
                        if path == file_path:
                            copy_file[i] = file_path
                    print(copy_file)
                    return render(request, "retake_word/retake_word_one5.html", {"recording": True,"val":val})
...
def letter_recording(request):
    if request.method == "POST":
        l_audio_file = request.session.setdefault('l_audio_file', [])
        l_responses = request.session.setdefault('l_responses', [])
        l_audio_file_name = request.session.setdefault('l_audio_file_name', [])
        l_copy_file = request.session.setdefault('l_copy_file', [])
        l_dataid = request.session.setdefault('l_dataid', [])



        if len(l_responses)==5:
            l_responses.clear()
            l_dataid.clear()

        print("check audio",l_audio_file)
        if "record" in request.POST:
            quest_val = request.POST.get('question')   
            print("question is ", quest_val)
            r = sr.Recognizer()
            with sr.Microphone() as source:
                ...
                #     with open(file_path, "wb") as f:
                #         f.write(audio.get_wav_data())
                #     for i, path in enumerate(l_copy_file):
                #         if path == file_path:
                #             l_copy_file[i] = file_path
                #     print(l_copy_file)
                #     print(file_path)
                ...
                try:
                    ...
                    with open(file_path, "wb") as f:
                        f.write(audio.get_wav_data())
                    l_audio_file_name.append(file_name)
                    l_audio_file.append(file_path)
                    request.session['file_path'] = file_path
                    if len(l_audio_file) < 5:
                        request.session['audio_recorded'] = True
                        with open('ParakhData_English.json') as f:
                            data = json.load(f)
                            Letter = None
                        ...
                        # msg = f"Recording {len(l_audio_file) + 1} of 5. Please speak the next letter."
                        return render(request, "fifteen_two_one.html", {"recording": True,"val":val})
                    elif len(l_audio_file) == 5:
                        msg = "You have recorded 5 files. Please click the Submit button to proceed."
                        request.session['audio_recorded'] = True
                        with open('ParakhData_English.json') as f:
                            data = json.load(f)
                        ...
                        return render(request, "fifteen_two_one_submit.html", {"val":val})
            ...
            # l_audio_file_name.clear()

            # l_responses.clear()
            # print(l_audio_file)
            # file = request.session.setdefault('file', [])
            # l_audio_file.clear()  # clear the l_audio_file list
            # print("res",l_responses)
            # file.clear()  # clear the file list
      ...
      ##  only delet the last one
            ...
            print(l_audio_file)
            file = request.session.setdefault('file', [])
            if len(l_audio_file) > 0:
                l_audio_file.pop()  # remove the last element from l_audio_file list
            print(l_audio_file)
            return render(request, "fifteen_two.html", {"msg": "", "val": get_random_letter(request)})

        elif "submit" in request.POST:
            l_audio_file = request.session['l_audio_file']
            quest_val = request.POST.get('question')
            file_path = request.session.get('file_path')
            if file_path is None:
            ...
            nomistake=[]
            wcpm=[]
            # for l_audio_file in l_audio_file:
            # file_path = os.path.join(settings.MEDIA_ROOT, l_audio_file)
            file_name = os.path.basename(file_path)
            url = 'http://3.7.133.80:8000/gettranscript/'
            files = [('audio', (file_name, open(file_path, 'rb'), 'audio/wav'))]
            payload = {'language': 'English', 'question': quest_val}
            ...
            if response.status_code == 200:
                l_responses.append(response.text)

                if len(l_responses)==5:
                    request.session['l_responses'] = l_responses
                    context = {'l_responses': l_responses}
...
def letter_answer(request):
    ...
    l_copy_file = request.session.setdefault('l_copy_file', [])
    l_audio_file_name = request.session.setdefault('l_audio_file_name', [])
    l_copy_file_name = request.session.setdefault('l_copy_file_name', [])
    print(l_audio_file_name)

    if len(l_audio_file)==5:
        l_copy_file= l_audio_file.copy()
    if len(l_audio_file_name)==5:
        l_copy_file_name = l_audio_file_name.copy()

    print("$$$",l_copy_file_name)
    request.session['l_copy_file_name'] = l_copy_file_name
    ...
    audio_urls = []
    if l_copy_file:
        for file_path in l_copy_file:
            if os.path.exists(file_path):
                file_name = os.path.basename(file_path)
                audio_url = request.build_absolute_uri(settings.MEDIA_URL + file_name)
                audio_urls.append(audio_url)
       
                # l_copy_file.remove(file_path)
        request.session['l_copy_file'] = l_copy_file
    
    text = []
    mis=[]
    wcpm=[]
    l_responses = request.session.get('l_responses', [])

    for item in l_responses:
        text.append(eval(item)['text'])
        print("text",text)
        mis.append(eval(item)['no_mistakes'])
        wcpm.append(eval(item)['wcpm'])
    ...
    context = { 'transcript': transcript,'text': text, 'l_copy_file': l_copy_file, 'audio_urls': audio_urls ,'mis':mis ,'flu':wcpm,'total_wcpm': total_wcpm,'total_mis':total_mis}
    if len(l_audio_file)==5:
        l_audio_file.clear()
    if len(l_audio_file_name)==5:
        l_audio_file_name.clear()
        print("audio",l_copy_file_name)
...
def next_letter(request):
    ...
    if request.method == "POST":
        l_copy_file_name = request.session.get('l_copy_file_name')

        l_copy_file_name = request.session.setdefault('l_copy_file_name', [])
        for file_name in l_copy_file_name:
            file_name_without_extension = file_name.split(".")[0]
                ...
                try:
                    ...
                    for i, path in enumerate(l_copy_file):
                        if path == file_path:
                            l_copy_file[i] = file_path
                    print(l_copy_file)
                    return render(request, "retake_letter/retake_letter_one.html", {"recording": True,"val":val})

                except sr.UnknownValueError:
                    print("Speech recognition could not understand audio")
                    error_msg = "Could not understand audio. Please try again."
                    return render(request, "fifteen_two.html", {"val": get_random_word(request), "error": error_msg})
                except sr.RequestError as e:
                    print(f"Could not request results from Google Speech Recognition service; {e}")
                    return render(request, "fifteen_two.html", {"val": get_random_word(request)}) 
            ...
            if response.status_code == 200:
                ...
                if len(l_responses)==5:
                    ...
                    if len(l_responses)==5:
                        ...
                        return redirect(url,context)
    return render(request, "retake_letter.html", {"recording": True })
                ...
                try:
                    ...
                    for i, path in enumerate(l_copy_file):
                        if path == file_path:
                            l_copy_file[i] = file_path
                    print(l_copy_file)
                    return render(request, "retake_letter/retake_letter_one2.html", {"recording": True,"val":val})
                ...
                try:
                    ...
                    for i, path in enumerate(l_copy_file):
                        if path == file_path:
                            l_copy_file[i] = file_path
                    print(l_copy_file)
                    return render(request, "retake_letter/retake_letter_one3.html", {"recording": True,"val":val})
                ...
                try:
                    ...
                    for i, path in enumerate(l_copy_file):
                        if path == file_path:
                            l_copy_file[i] = file_path
                    print(l_copy_file)
                    return render(request, "retake_letter/retake_letter_one4.html", {"recording": True,"val":val})
                ...
                try:
                    ...
                    for i, path in enumerate(l_copy_file):
                        if path == file_path:
                            l_copy_file[i] = file_path
                    print(l_copy_file)
                    return render(request, "retake_letter/retake_letter_one5.html", {"recording": True,"val":val})
...
#                 f.write(audio.get_wav_data())
#             print("Audio file saved at", file_path)
#             l_audio_files = request.session.get('audio_files', [])
